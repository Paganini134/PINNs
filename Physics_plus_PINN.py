...
# Cell 2: Updated PINN for PCM Melt Fraction Prediction with Normalization and Larger Architecture
...
# PINN Model: x (position), t (time), T_boundary as inputs --> Predict temperature T(x,t)
class PINN_PCM(nn.Module):
    ...
    def forward(self, x, t, T_b, params):  # Normalize inputs inside forward
        x_norm = normalize_input(x, 0.0, params['length'])
        t_norm = normalize_input(t, 0.0, params['t_max'])
        T_b_norm = normalize_input(T_b, params['T_b_min'], params['T_b_max'])
        inputs = torch.cat([x_norm, t_norm, T_b_norm], dim=1)
        T_pred = self.net(inputs)
        # Output raw temperature in °C (no normalization on output)
        return T_pred


# Physics: effective properties (same as before)
def melt_fraction(T, T_solidus, T_liquidus):
    zero = torch.zeros_like(T)
    one = torch.ones_like(T)
    mf = torch.where(
        T <= T_solidus,
        zero,
        torch.where(T >= T_liquidus, one, (T - T_solidus) / (T_liquidus - T_solidus)),
    )
    return mf


def cp_eff(T, mf, cp_solid, cp_liquid, L_fusion, T_solidus, T_liquidus):
    cp = cp_solid + mf * (cp_liquid - cp_solid)
    latent = torch.where(
        (T > T_solidus) & (T < T_liquidus),
        L_fusion / (T_liquidus - T_solidus),
        torch.zeros_like(T),
    )
    return cp + latent


def k_eff(mf, k_solid, k_liquid):
    return k_solid + mf * (k_liquid - k_solid)


# Loss (physics-informed, boundaries, initial) unchanged except pass params
def pinn_loss(model, x, t, T_b, params):
    x.requires_grad_()
    t.requires_grad_()
    T_pred = model(x, t, T_b, params)
    mf = melt_fraction(T_pred, params["T_solidus"], params["T_liquidus"])
    cp = cp_eff(
        T_pred,
        mf,
        params["cp_solid"],
    ...
    k = k_eff(mf, params["k_solid"], params["k_liquid"])
    ...
    T_x = torch.autograd.grad(
        T_pred, x, grad_outputs=torch.ones_like(T_pred), create_graph=True
    )[0]
    T_xx = torch.autograd.grad(
        T_x, x, grad_outputs=torch.ones_like(T_x), create_graph=True
    )[0]
    pde = T_t - (1 / (params["rho"] * cp)) * (k * T_xx)

    # Boundary condition @ x=0, x=L
    x0 = torch.zeros_like(x)
    xL = torch.full_like(x, params["length"])
    T0 = model(x0, t, T_b, params)
    TL = model(xL, t, T_b, params)
    loss_bc = torch.mean((T0 - T_b) ** 2) + torch.mean((TL - T_b) ** 2)

    # Initial condition @ t=0
    t0 = torch.zeros_like(t)
    T_init = model(x, t0, T_b, params)
    loss_ic = torch.mean((T_init - params["T_initial"]) ** 2)
...
# Cell 3: Updated Training, Validation, Testing with Learning Rate Scheduler and Normalized Inputs

import torch.optim as optim
from sklearn.model_selection import train_test_split
import matplotlib.pyplot as plt

# Data sampling unchanged but pass new params as before
def sample_training_points(N_colloc=12000, N_bc=800, N_ic=800, params=None):
    x_colloc = torch.rand(N_colloc, 1) * params['length']
    t_colloc = torch.rand(N_colloc, 1) * params['t_max']
    T_b_colloc = torch.rand(N_colloc, 1) * (params['T_b_max'] - params['T_b_min']) + params['T_b_min']

    t_bc = torch.rand(N_bc, 1) * params['t_max']
    x_bc0 = torch.zeros(N_bc, 1)
    x_bcL = torch.full((N_bc, 1), params['length'])
    T_b_bc = torch.rand(N_bc, 1) * (params['T_b_max'] - params['T_b_min']) + params['T_b_min']

    t_ic = torch.zeros(N_ic, 1)
    x_ic = torch.rand(N_ic, 1) * params['length']
    T_b_ic = torch.rand(N_ic, 1) * (params['T_b_max'] - params['T_b_min']) + params['T_b_min']

    x_bc = torch.cat([x_bc0, x_bcL], dim=0)
    t_bc = torch.cat([t_bc, t_bc], dim=0)
    T_b_bc = torch.cat([T_b_bc, T_b_bc], dim=0)

    return (x_colloc, t_colloc, T_b_colloc), (x_bc, t_bc, T_b_bc), (x_ic, t_ic, T_b_ic)


# Params unchanged; just longer training and scheduler added
params = {
    'length': 0.1,
    't_max': 600.0,
    'T_b_min': 20.0,
...
# Sample points
(X_colloc, T_colloc, Tb_colloc), (X_bc, T_bc, Tb_bc), (X_ic, T_ic, Tb_ic) = sample_training_points(params=params)

# Train/Val split for BC and IC points
X_bc_train, X_bc_val, T_bc_train, T_bc_val, Tb_bc_train, Tb_bc_val = train_test_split(
    X_bc, T_bc, Tb_bc, test_size=0.2, random_state=42)
X_ic_train, X_ic_val, T_ic_train, T_ic_val, Tb_ic_train, Tb_ic_val = train_test_split(
    X_ic, T_ic, Tb_ic, test_size=0.2, random_state=42)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = PINN_PCM([3, 128, 128, 128, 64, 1]).to(device)  # Bigger model
...
for epoch in range(n_epochs):
    model.train()
    optimizer.zero_grad()
    loss_pde = pinn_loss(model, X_colloc.to(device), T_colloc.to(device), Tb_colloc.to(device), params)
    loss_bc = pinn_loss(model, X_bc_train.to(device), T_bc_train.to(device), Tb_bc_train.to(device), params)
    loss_ic = pinn_loss(model, X_ic_train.to(device), T_ic_train.to(device), Tb_ic_train.to(device), params)
    ...
    model.eval()
    # No torch.no_grad() to keep gradients for pinn_loss autograd
    val_loss_bc = pinn_loss(model, X_bc_val.to(device), T_bc_val.to(device), Tb_bc_val.to(device), params)
    val_loss_ic = pinn_loss(model, X_ic_val.to(device), T_ic_val.to(device), Tb_ic_val.to(device), params)
    val_loss = val_loss_bc + val_loss_ic
    val_losses.append(val_loss.item())

    if epoch % 200 == 0:
        print(f"Epoch {epoch}, Train Loss: {loss.item():.2e}, Val Loss: {val_loss.item():.2e}")
...
# Inference plot (unchanged)
def plot_mf_vs_position(model, params, T_boundary_test=32.0, time_test=200.0):
    model.eval()
    x_test = torch.linspace(0, params['length'], 200).unsqueeze(1).to(device)
    t_test = torch.full_like(x_test, time_test)
    Tb_test = torch.full_like(x_test, T_boundary_test)
    with torch.no_grad():
        T_pred = model(x_test, t_test, Tb_test, params)
        mf_pred = melt_fraction(T_pred, params['T_solidus'], params['T_liquidus'])
        x_mm = x_test.cpu().numpy().flatten() * 1000
        mf_np = mf_pred.cpu().numpy().flatten()
    import matplotlib.pyplot as plt
    plt.figure()
    plt.plot(x_mm, mf_np, label=f'T_boundary={T_boundary_test}°C at t={time_test}s')
    plt.xlabel("Position (mm)")
    plt.ylabel("Melt Fraction")
    plt.title("MF vs Position (PINN Prediction)")
    plt.ylim([-0.05, 1.05])
    plt.grid(alpha=0.3)
    plt.legend()
    plt.show()
...
plot_mf_vs_position(model, params, T_boundary_test=32.0, time_test=200.0)
